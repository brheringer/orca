...
RSpec.describe Types::MutationType do
  ...
  describe "update account" do
    ...
    let(:query) do
      %(mutation UpdateAccountMutation($id: ID!, $structure: String!, $name: String!, $kind: AccountKind!) {
    ...
    subject(:result) do #significa: o subject (objeto do teste) vai ser o resultado da query (json); sem isso, seria QueryType.new
      OrcaSchema.execute(query, 
          variables: {id:account.id, structure:'1.1', name:'account name', kind: Types::AccountKind.CREDIT},
    ...
    it "should update the account" do
      ...
      expect(result['data']['updateAccount']['account']['kind']).to eq(Account.kinds.CREDIT)
    end
  end
